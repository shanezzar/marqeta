//
// BaseAchResponseModel.swift
//
// Generated by swagger-codegen
// https://github.com/swagger-api/swagger-codegen
//

import Foundation



public struct BaseAchResponseModel: Codable {

    /** ACH account identifier appended to the bank account number. */
    public var accountSuffix: String
    /** Type of account. */
    public var accountType: String
    /** Specifies whether the account is active. */
    public var active: Bool
    /** Name of the bank holding the account. This field is returned if it exists in the resource. */
    public var bankName: String?
    /** Date and time when the resource was created, in UTC. */
    public var createdTime: Date
    /** Date and time when the account was sent for verification, in UTC. This field is returned if it exists in the resource. */
    public var dateSentForVerification: Date?
    /** Date and time when the account was verified, in UTC.  This field is returned if it exists in the resource. */
    public var dateVerified: Date?
    /** If there are multiple funding sources, this field specifies which source is used by default in funding calls. If there is only one funding source, the system ignores this field and always uses that source. */
    public var isDefaultAccount: Bool?
    /** Date and time when the resource was last modified, in UTC. */
    public var lastModifiedTime: Date
    /** Name on the ACH account. */
    public var nameOnAccount: String
    public var partner: String?
    public var partnerAccountLinkReferenceToken: String?
    /** The unique identifier of the funding source. */
    public var token: String
    /** Free-form text field for holding notes about verification. This field is returned only if &#x60;verification_override &#x3D; true&#x60;. */
    public var verificationNotes: String?
    /** Allows the ACH funding source to be used regardless of its verification status. This field is returned if it exists in the resource. */
    public var verificationOverride: Bool?
    /** Account verification status. This field is returned if it exists in the resource. */
    public var verificationStatus: String?

    public init(accountSuffix: String, accountType: String, active: Bool, bankName: String? = nil, createdTime: Date, dateSentForVerification: Date? = nil, dateVerified: Date? = nil, isDefaultAccount: Bool? = nil, lastModifiedTime: Date, nameOnAccount: String, partner: String? = nil, partnerAccountLinkReferenceToken: String? = nil, token: String, verificationNotes: String? = nil, verificationOverride: Bool? = nil, verificationStatus: String? = nil) {
        self.accountSuffix = accountSuffix
        self.accountType = accountType
        self.active = active
        self.bankName = bankName
        self.createdTime = createdTime
        self.dateSentForVerification = dateSentForVerification
        self.dateVerified = dateVerified
        self.isDefaultAccount = isDefaultAccount
        self.lastModifiedTime = lastModifiedTime
        self.nameOnAccount = nameOnAccount
        self.partner = partner
        self.partnerAccountLinkReferenceToken = partnerAccountLinkReferenceToken
        self.token = token
        self.verificationNotes = verificationNotes
        self.verificationOverride = verificationOverride
        self.verificationStatus = verificationStatus
    }

    public enum CodingKeys: String, CodingKey { 
        case accountSuffix = "account_suffix"
        case accountType = "account_type"
        case active
        case bankName = "bank_name"
        case createdTime = "created_time"
        case dateSentForVerification = "date_sent_for_verification"
        case dateVerified = "date_verified"
        case isDefaultAccount = "is_default_account"
        case lastModifiedTime = "last_modified_time"
        case nameOnAccount = "name_on_account"
        case partner
        case partnerAccountLinkReferenceToken = "partner_account_link_reference_token"
        case token
        case verificationNotes = "verification_notes"
        case verificationOverride = "verification_override"
        case verificationStatus = "verification_status"
    }

}
